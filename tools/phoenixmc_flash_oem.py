...
if not IMAGE_PATH.exists():
    print(f"ERROR: OEM dump not found: {IMAGE_PATH}")
    sys.exit(1)
print(f"OEM dump: {IMAGE_PATH}  ({IMAGE_PATH.stat().st_size:,} bytes)")

# ── Set FLASH length=4MB and address=0 ────────────────────────────────────────
...
# ── Click 写入 — expect file-open dialog ──────────────────────────────────────
...
if not filename_edit:
    print("ERROR: no filename Edit field in file dialog")
    sys.exit(1)
...
print("\nMonitoring flash progress (4 MB at 115200 baud ≈ 5–6 min)...")
            ...
            try:
                ...
                if any(
                    ...
                    if "write ok" in tl or any(
                        ...
                        print("\nDone! OEM firmware flashed successfully.")
                        sys.exit(0)
