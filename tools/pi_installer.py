...
def collect_install_config():
    ...
    country = _prompt_validated(
        "  WiFi country code (ISO 3166 alpha-2, e.g. GB, US, DE, NL) [GB]: ",
        validate_country_code,
        default="GB",
    )
    timezone = _prompt_validated(
        "  Timezone (IANA, e.g. Europe/London, America/New_York) [Europe/London]: ",
        validate_timezone,
...
def _render_firstrun(cfg):
    ...
    country = _shell_escape(cfg.get("country") or "GB")
    timezone = _shell_escape(cfg.get("timezone") or "Europe/London")
