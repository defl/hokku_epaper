...
static char   _mock_nvs_wifi_ssid[33]   = {0};
static char   _mock_nvs_wifi_pass[65]   = {0};
...
static inline int nvs_get_str(nvs_handle_t h, const char *k, char *v, size_t *l) {
    (void)h;
    const char *src = NULL;
    if      (strcmp(k, "wifi_ssid")   == 0) src = _mock_nvs_wifi_ssid;
    else if (strcmp(k, "wifi_pass")   == 0) src = _mock_nvs_wifi_pass;
