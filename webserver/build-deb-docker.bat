...
docker run --rm ^
    ...
    debian:bookworm ^
    bash -c "set -e && apt-get update -qq && apt-get install -y --no-install-recommends build-essential debhelper dh-python python3 python3-setuptools && dpkg-buildpackage -us -uc -b"
