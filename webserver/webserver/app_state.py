"""Mutable holder for the live config / manager / scheduler trio.
...
class AppState:
    """Thread-safe container for the three live application objects.

    Routes and the watcher read ``state.manager`` / ``state.scheduler`` /
    ``state.config`` directly.  The attributes are written only inside
    ``reload()``, which holds ``_lock`` for the duration of the swap — a
    microsecond-level critical section.  Readers do *not* need to acquire the
    lock: they grab a local reference at the start of a request and work with
    that snapshot, which is safe under the GIL and our single-writer pattern.
    """
    ...
    def reload(self, new_config: AppConfig) -> None:
        """Rebuild manager + scheduler from *new_config* and swap atomically.
        ...
        new_manager = ImageManager(new_config)
