...
from webserver.display_constants import VISUAL_H, VISUAL_W
...
from webserver.panel_format import indices_to_panel_bytes, indices_to_preview_rgb
...
_PREP_STANDARD = (0.5, 0.85, 1.0, 1.1, 1.3)
_VIVID = (5.0, 15.0)
_SAT = (1.25, 5.0, 15.0)
_HUE_LUT_CUTOFF_DEG = 95.0
_HUE_LUT_NEUTRAL_CHROMA = 8.0
...
class ImageConfig:
    ...
    def cache_slug(self) -> str:
        """Path-safe short fingerprint of the full ingest + dither pipeline."""
...
def _ic(
    ...
    *,
    ...
    hue_cutoff_deg: float = _HUE_LUT_CUTOFF_DEG,
    neutral_chroma: float = _HUE_LUT_NEUTRAL_CHROMA,
    color_enhance: float,
    use_adaptive_saturate: bool,
    saturate_max_enhance: float,
    saturate_low_chroma_thresh: float,
    ...
    dither_cfg = DitherConfig(
        algorithm=algorithm,  # type: ignore[arg-type]
        lut_name=lut_name,  # type: ignore[arg-type]
        serpentine=serpentine,
        hue_cutoff_deg=hue_cutoff_deg,
        neutral_chroma=neutral_chroma,
    )
    return ImageConfig(
        dither=dither_cfg,
        prepare_autocontrast_cutoff=_PREP_STANDARD[0],
        prepare_gamma=_PREP_STANDARD[1],
        prepare_brightness=_PREP_STANDARD[2],
        prepare_contrast=_PREP_STANDARD[3],
        prepare_sharpness=_PREP_STANDARD[4],
        color_enhance=color_enhance,
        use_adaptive_saturate=use_adaptive_saturate,
        saturate_max_enhance=saturate_max_enhance,
        saturate_low_chroma_thresh=saturate_low_chroma_thresh,
        saturate_high_chroma_thresh=saturate_high_chroma_thresh,
        scale_chroma=scale_chroma,
        adaptive_vivid=adaptive_vivid,
        vivid_chroma_low=_VIVID[0],
        vivid_chroma_high=_VIVID[1],
    )


PRESET_IMAGE_CONFIGS: dict[str, ImageConfig] = {
    "floyd_steinberg": _ic(
        "floyd_steinberg", "euclidean",
        color_enhance=1.2,
    ...
    "floyd_steinberg_hue_aware": _ic(
        "floyd_steinberg", "hue_aware",
        color_enhance=1.25,
        use_adaptive_saturate=True,
        saturate_max_enhance=_SAT[0],
        saturate_low_chroma_thresh=_SAT[1],
        saturate_high_chroma_thresh=_SAT[2],
        scale_chroma=False,
        adaptive_vivid=True,
    ),
    "atkinson": _ic(
...
def default_image_config() -> ImageConfig:
    return PRESET_IMAGE_CONFIGS["atkinson_hue_aware"]
...
def preset_name_for_image(img: ImageConfig) -> str | None:
    for name, preset in PRESET_IMAGE_CONFIGS.items():
        if replace(preset, dither=replace(preset.dither, serpentine=img.dither.serpentine)) == img:
            return name
    return None
...
def prepare_panel_canvas(
    ...
    canvas = apply_prepare_enhancements(canvas, image_cfg)
...
class ImageConverter:
    """Full image path → (raw panel bytes, preview PNG bytes)."""

    def convert(self, img_path: Path, config: "AppConfig") -> tuple[bytes, bytes]:
        ...
        canvas, padding_mask = prepare_panel_canvas(img, image_cfg, config.orientation)
        ...
        preview_bytes = preview_panel_rgb_to_png(preview_rgb, config.orientation)
