...
**CLAHE face protection** — when face detection is enabled, an additional toggle controls whether detected face regions are shielded from the CLAHE local-contrast step. CLAHE can sharply alter skin tones; this toggle keeps face regions untouched while still applying CLAHE to the rest of the image. The boundary between protected and unprotected regions is feathered with a Gaussian blur to avoid a hard line at the edge of the face. Enabled by default when face detection is on.
