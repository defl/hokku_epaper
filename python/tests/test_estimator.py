...
from hokku.webserver.image_manager_abstract import ConversionProgress, ImageRecord
...
def _manager_with(
    app_config: AppConfig, records: list[ImageRecord], progress: ConversionProgress
...
def test_inflight_images_excluded_from_estimate(app_config: AppConfig):
    ...
    assert eta == pytest.approx(1.0), f"expected 1.0 s (excluding inflight b.png), got {eta}"
