"""Tests for the screen-flashing feature (hokku.screens.huessen_epf1301 + web job manager).
...
def test_nvs_build_read_round_trip():
    ...
    blob = huessen_epf1301.build_nvs_binary(cfg)
    assert len(blob) == huessen_epf1301.NVS_SIZE
    back = huessen_epf1301.read_nvs(blob)
...
def test_nvs_unavailable_raises(monkeypatch):
    monkeypatch.setattr(huessen_epf1301.nvs, "nvs_tool_available", lambda: False)
    with pytest.raises(huessen_epf1301.NvsToolUnavailable):
        huessen_epf1301.nvs.build_nvs_binary({"wifi_ssid1": "x", "image_url": "y"})


# ── huessen_epf1301.device.parse_device_state ─────────────────────────────────────────
...
def test_parse_device_state_detects_firmware_and_version():
    ...
    state = huessen_epf1301.parse_device_state(b"", header, release_header=header)
...
def test_parse_device_state_blank_device():
    state = huessen_epf1301.parse_device_state(
        None, b"\xff" * 256, release_header=_make_app_header()
    )
...
def test_parse_device_state_reads_config():
    ...
    nvs = huessen_epf1301.build_nvs_binary(
        {"wifi_ssid1": "Net", "image_url": "u", "screen_name": "Den"}
    )
    state = huessen_epf1301.parse_device_state(nvs, _make_app_header())
    assert state["config_version_ok"] is True
    assert state["config"]["screen_name"] == "Den"


# ── huessen_epf1301.firmware ──────────────────────────────────────────────────────────
...
def test_merged_firmware_file_none_when_empty(tmp_path):
    assert huessen_epf1301.merged_firmware_file(tmp_path) is None
...
def test_job_manager_runs_and_streams(monkeypatch):
    ...
    monkeypatch.setattr(flashing.huessen_epf1301, "flash_device", fake_flash)
    mgr = flashing.FlashJobManager()
    fw = Path("fw.bin")
...
def test_job_manager_rejects_concurrent(monkeypatch):
    ...
    monkeypatch.setattr(flashing.huessen_epf1301, "flash_device", slow_flash)
    mgr = flashing.FlashJobManager()
    fw = Path("fw.bin")
...
def test_job_manager_records_error(monkeypatch):
    ...
    monkeypatch.setattr(flashing.huessen_epf1301, "flash_device", boom)
    mgr = flashing.FlashJobManager()
